...
# **************************************************************
set neorv32_home ../..
set AlohaHE_Common_home ../../../thesis_Aloha-HE/Aloha-HE_Common
set AlohaHE_IP_home ../../../thesis_Aloha-HE/Aloha-HE_Kintex/Aloha-HE_Kintex.srcs/sources_1/ip
...
# additional user sources (VHDL, Verilog, System-Verilog)
set SRC_USER {}
proc get_files_recursive {dir pattern} {
    ...
    foreach file_or_dir [glob -nocomplain -directory $dir *] {
        set full_path "$file_or_dir"
        # puts "Searching current path:"
        # puts $full_path

        # If it's a directory, recursively search it
        if {[file isdirectory $full_path]} {
            get_files_recursive $full_path $pattern
        } elseif {[file isfile $full_path] && [string match $pattern $full_path]} {
            # If it's a file and matches the pattern, add it to the list
            lappend SRC_USER $full_path
        }
    }
}
get_files_recursive $AlohaHE_Common_home *.v
get_files_recursive $AlohaHE_Common_home *.sv
get_files_recursive $AlohaHE_Common_home *.vh
get_files_recursive $AlohaHE_Common_home *.coe
get_files_recursive $AlohaHE_IP_home *.xci
# Print the new files for verification, one per line
puts "Aloha-HE source files:"
foreach file $SRC_USER {
    puts $file
}
