				  ...
				  const uint32_t* log_q)
{
	...
	int32_t log_scale_update = log_scale - 52 - 1023 - 13; // -13 for scaling factor of 1/N
	if(log_scale_update < 0)
		log_scale_update += 4096;
	...
	for(uint8_t modulus_index = 0; modulus_index < num_moduli; ++modulus_index)
	{
		...
		instructions_encrypt[1] = getRNSInstructionWord(log_scale_update, log_q[modulus_index], rns_modulus_rom_indices[modulus_index], qm[modulus_index]);
